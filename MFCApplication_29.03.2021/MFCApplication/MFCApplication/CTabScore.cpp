...
void CTabScore::LoadDataFromFile()
{
	...
	for (auto it = mapScore.begin(); it != mapScore.end(); it++)
	{
		...
		ScoreStruct scoreStruct = {it->first, atoi(it->second[0]), it->second[1], it->second[2], it->second[3], it->second[4]};
		m_listScore.push_back(scoreStruct);

		nItemIndex = m_listCtrl.InsertItem(nCount, oLib.IntToCString(scoreStruct.nIdStudent));
