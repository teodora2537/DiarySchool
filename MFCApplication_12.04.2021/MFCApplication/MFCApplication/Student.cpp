...
void CStudent::AvgScoreByAllSubject(list<ReferenceStruct>& m_listReference) {
	...
	CDatabase dbTest;
	CString SqlString, ConnectionString;
	CString m_strSubject, m_strFN, m_strLN, m_strStudentId, m_strSubjectId;
	CDBVariant vtval;
	ReferenceStruct refStruct;
	// Allocate the recordset
	CRecordset recset(&db);
	...
	//sql string for all
	SqlString = "SELECT Score.student_id, Score.subject_id, Student.first_name, Student.last_name FROM Student INNER JOIN Score ON Score.student_id = Student.id;";
	recset.Open(CRecordset::forwardOnly, SqlString, CRecordset::readOnly);
	...
	while (!recset.IsEOF()) {
		...
		/*Avg score*/
		SqlString = "SELECT AVG(score) FROM Score WHERE Score.student_id = '" + m_strStudentId + "';";
		recsetTest.Open(CRecordset::forwardOnly, SqlString, CRecordset::readOnly);
		recsetTest.GetFieldValue((short)0, vtval);
		avgScore = vtval.m_lVal;
		recsetTest.Close();
		...
		{
			refStruct = { atoi(m_strStudentId), m_strFN + " " + m_strLN, oLib.IntToCString(avgScore) };
			m_listReference.push_back(refStruct);
		...
		{
			if (i->nIdStudent == atoi(m_strStudentId))
			{
				isFind = true;
				break;
			}
...
void CStudent::remedialExaminationBySub(list<ReferenceStruct>& m_listReference) {
	...
	while (!recset.IsEOF()) {
		...
		refStruct.strClm0 = m_strFN + " " + m_strLN;
		refStruct.strClm1 = m_strSubject;
		m_listReference.push_back(refStruct);
