...
*/
void CTabReference::EditScoreItem(CScoreData& oScoreData) {
	///////
	Library lib;
	int row = oScoreData.GetIdScore() - 1;

	CString m_cstrClassNum;
	m_cstrClassNum.Format(L"%d", oScoreData.GetClassNum());
	CString m_cstrSubject(oScoreData.GetSubject().c_str());
	CString m_cstrDate(oScoreData.GetDate().c_str());
	CString m_cstrScore;
	m_cstrScore.Format(L"%d", oScoreData.GetScore());

	m_listCtrl.SetItemText(row, 0, m_cstrClassNum);
	m_listCtrl.SetItemText(row, 1, m_cstrSubject);
	m_listCtrl.SetItemText(row, 2, m_cstrDate);
	m_listCtrl.SetItemText(row, 3, m_cstrScore);
	///////
}
