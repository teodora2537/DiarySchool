...
void CStudent::GetLastId(CStudentData& oStudent) {
	try 
	{
		CString strId;
		CRecordset recset(&g_dbConnection);
		recset.Open(CRecordset::forwardOnly, "SELECT max(id) as lastId FROM Student", CRecordset::readOnly);
		recset.GetFieldValue("lastId", strId);
		oStudent.m_iClassNumber = atoi(strId)+1;
	}
	catch (exception e) {
