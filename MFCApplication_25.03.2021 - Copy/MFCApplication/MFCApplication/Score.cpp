...
void CScore::PrintScore(map<int, vector<CString>>& mapScore)
{
	...
	while (getline(m_file, m_strText))
	{
		...
		while (true)
		{
			...
			m_strToken = (m_strText.substr(0, m_sizeTPosition).c_str());
			m_iCount++;
			if (m_iCount == 1) oCurrentScore.m_iIdScore = atoi(m_strToken);
			else if (m_iCount == 2) oCurrentScore.m_iClassNum = atoi(m_strToken);
		...
		if (m_mapAllStudent.size() == 0)
			return;

		mapScore[oCurrentScore.m_iIdScore].push_back(oLib.IntToCString(oCurrentScore.m_iClassNum));
		mapScore[oCurrentScore.m_iIdScore].push_back(m_mapAllStudent[oCurrentScore.m_iClassNum][0] + " " + m_mapAllStudent[oCurrentScore.m_iClassNum][1]);
		mapScore[oCurrentScore.m_iIdScore].push_back(oCurrentScore.m_strSubject);
		mapScore[oCurrentScore.m_iIdScore].push_back(oLib.IntToCString(oCurrentScore.m_iScore));
		mapScore[oCurrentScore.m_iIdScore].push_back(oCurrentScore.m_strDate);
	}
	m_file.close();
}